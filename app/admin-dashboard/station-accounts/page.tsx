...
export default function StationAccountsManagement() {
  ...
  const fetchAccounts = async () => {
    ...
    const res = await fetch('http://localhost:8000/api/admin/station-accounts/', { headers: { Authorization: `Bearer ${token}` } });
    if (res.ok) setAccounts(await res.json());
    setLoading(false);
  };
  const fetchStations = async () => {
    const token = localStorage.getItem('access_token');
    const res = await fetch('http://localhost:8000/api/admin/fire-stations/', { headers: { Authorization: `Bearer ${token}` } });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    const token = localStorage.getItem('access_token');
    const res = await fetch('http://localhost:8000/api/admin/station-accounts/', {
      method: 'POST',
  ...
  const handleEditSubmit = async (e: React.FormEvent) => {
    ...
    const token = localStorage.getItem('access_token');
    const res = await fetch(`http://localhost:8000/api/admin/station-accounts/${editingAccount.id}/`, {
      method: 'PATCH',
      headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
  ...
  const handleDelete = async (id: number) => {
    ...
    const token = localStorage.getItem('access_token');
    await fetch(`http://localhost:8000/api/admin/station-accounts/${id}/`, { method: 'DELETE', headers: { Authorization: `Bearer ${token}` } });
    fetchAccounts();
  };
